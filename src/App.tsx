...
import {
  COGNITIVE_FAMILIES, FAMILY_LABELS, LOGIC_FAMILIES, NUMBER_FAMILIES,
  ...
  type VisualSpec, type VisualToken,
} from './types'
...
const DAILY_GOAL = 3
type Preferences = { difficulty: Difficulty; duration: number }
...
const familyIcons: Record<ExerciseFamily, typeof Sigma> = {
  ...
  'data-sprint': Database, 'debug-scan': Target, 'memory-grid': Grid2X2,
  'pattern-recall': Grid2X2, 'tile-sequence': ListOrdered, 'arrow-shift': Target, spatial: RotateCw, 'route-planner': Route,
}
...
const familyDescriptions: Record<ExerciseFamily, string> = {
  ...
  'memory-grid': 'Retain larger patterns, counts and precise locations.',
  ...
  spatial: 'Compose rotations, reflections and positional transforms.',
...
function loadPreferences(): Preferences {
  ...
  } catch { return { difficulty: 'Adaptive', duration: 60 } }
}
...
export function buildSessionResult(config: SessionConfig, attempts: Attempt[], elapsed: number): SessionResult {
  ...
  const speed = completed.length
    ? completed.reduce((sum, attempt) => sum + Math.min(1, attempt.exercise.responseTargetMs / Math.max(1, attempt.responseMs)), 0) / completed.length
    : 0
  ...
  return {
    ...
    medianMs: median(responseTimes), score: Math.round((accuracy * .65 + speed * .22 + volume * .13) * 100), breakdown,
  }
}
  ...
  return <div className="page practice-page">
    ...
    <section className="builder-layout">
      ...
      <aside className="start-panel">
        ...
        <button className="primary-button light wide" onClick={() => onStart({ track, label: recordLabel, families: activeFamilies, difficulty: preferences.difficulty, duration: preferences.duration })}><Play size={17} fill="currentColor" /> {untimed ? 'Start practice' : 'Begin drill'}</button>
      </aside>
    </section>
...
function Glyph({ token, size = 64 }: { token: VisualToken; size?: number }) {
  const count = token.count || 1
  const positionOffset: Record<NonNullable<VisualToken['position']>, [number, number]> = { center: [0, 0], top: [0, -18], right: [18, 0], bottom: [0, 18], left: [-18, 0] }
  ...
  return <svg className="glyph" viewBox="0 0 100 100" width={size} height={size} aria-hidden="true">{centers.slice(0, count).map(([x, y], index) => <g key={index} transform={`rotate(${token.rotation || 0} ${x} ${y})`}>
...
function VisualPrompt({ visual, hidden = false }: { visual: VisualSpec; hidden?: boolean }) {
  if (visual.kind === 'matrix' || visual.kind === 'tiles') return <div className={`visual-grid ${visual.kind}`} style={{ gridTemplateColumns: `repeat(${visual.columns}, 1fr)` }}>{visual.cells.map((cell, index) => <div className="visual-cell" key={index}>{cell ? <Glyph token={cell} /> : <span className="missing-tile">?</span>}{visual.kind === 'tiles' && <small>{index + 1}</small>}</div>)}</div>
  ...
  if (visual.kind === 'memory') return <div className="memory-grid" style={{ gridTemplateColumns: `repeat(${visual.size}, 1fr)` }}>{Array.from({ length: visual.size * visual.size }, (_, index) => <span key={index} className={!hidden && visual.cells.includes(index) ? 'lit' : ''} />)}</div>
...
function ExercisePrompt({ exercise, onAnswer, value, onValue, feedback, disabled = false }: { exercise: Exercise; onAnswer: (value: AnswerValue) => void; value: string; onValue: (value: string) => void; feedback?: 'correct' | 'incorrect' | null; disabled?: boolean }) {
  ...
  const choiceClass = [
    'choice-grid',
    exercise.options?.some((option) => option.visual) ? 'visual-options' : '',
    ...
    exercise.family === 'memory-grid' && exercise.options?.some((option) => option.label.length > 8) ? 'memory-options' : '',
  ].filter(Boolean).join(' ')
...
function PracticeSession({ config, onFinish, onExit }: { config: SessionConfig; onFinish: (config: SessionConfig, attempts: Attempt[], elapsed: number) => void; onExit: () => void }) {
  ...
  const answer = (given: AnswerValue) => { if (feedback || paused) return; const correct = isCorrect(exercise, given); const responseMs = Date.now() - questionStartedAt.current; const attempt: Attempt = { exercise, given, correct, skipped: false, responseMs }; const updated = [...attemptsRef.current, attempt]; attemptsRef.current = updated; setAttempts(updated); setFeedback(correct ? 'correct' : 'incorrect'); setStreak((current) => correct ? current + 1 : 0); const nextLevels = { ...levels }; if (config.difficulty === 'Adaptive') { if (correct && responseMs < exercise.responseTargetMs) nextLevels[exercise.family] = Math.min(10, levels[exercise.family] + 1); if (!correct) nextLevels[exercise.family] = Math.max(1, levels[exercise.family] - 1); setLevels(nextLevels) } const feedbackMs = exercise.answer.kind === 'sequence' && !correct ? 950 : correct ? 650 : 1200; window.setTimeout(() => next(nextLevels), feedbackMs) }
  useEffect(() => { const handler = (event: KeyboardEvent) => { if (exercise.answer.kind !== 'choice' || feedback || paused) return; const option = exercise.options?.[Number(event.key) - 1]; if (option) answer(option.id) }; window.addEventListener('keydown', handler); return () => window.removeEventListener('keydown', handler) })
  ...
  return <div className="session-screen">
    ...
    <main className={`session-main ${feedback || ''}`}><div className="session-metric left-metric"><span><Flame size={18} /> Streak</span><strong>{streak}</strong><small>{streak >= 5 ? 'On fire' : 'Build momentum'}</small></div><div className="session-metric right-metric"><span><Target size={18} /> Accuracy</span><strong>{attempts.length ? Math.round(correctCount / attempts.length * 100) : 100}%</strong><small>{correctCount} of {attempts.length} correct</small></div><section className="question-area exercise-question-area"><div className={`timer-display ${untimed ? 'practice-timer' : ''}`}><Clock3 size={18} /><span>{formatTime(untimed ? elapsed : remaining)}</span>{untimed && <small>elapsed</small>}</div><p className="question-count">{exercise.label} <span>·</span> Level {levels[exercise.family]} <span>·</span> {levelName(levels[exercise.family])}</p><ExercisePrompt key={exercise.id} exercise={exercise} onAnswer={answer} value={value} onValue={setValue} feedback={feedback} disabled={paused || !!feedback} /><div className={`feedback-copy explanation ${feedback ? 'show' : ''}`}>{feedback === 'correct' ? `Correct. ${exercise.explanation}` : feedback === 'incorrect' ? `Not quite. ${exercise.explanation}` : ''}</div></section><div className="session-footer-note"><Keyboard size={15} /> Use the keyboard or click an answer</div></main>
